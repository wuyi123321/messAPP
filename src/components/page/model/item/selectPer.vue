...
<template>
  <div class="main">
    <div class="header">
      <mu-appbar title="选择用户" style="text-align: center">
      ...
      <mu-drawer :open="draweropen" :docked="false" :right="true" width="85%"  @close="draweropen=false">
        <div class="searRight">
            ...
            <div class="srcontent">
              <div>
                <mu-text-field label="员工编号"   hintText="请输入查询内容"   v-model="searchAll['empDescCust.empNo']"  labelFloat/>
                <mu-text-field label="员工姓名"   hintText="请输入借用部门"    v-model="searchAll['empDescCust.empName']"     labelFloat/>
              </div>
            </div>
        </div>
      ...
      <div class="search">
        ...
        >
          <mu-menu-item title="员工编号" @click="searchitem('员工编号','empDescCust.empNo')"/>
          <mu-menu-item title="员工姓名" @click="searchitem('员工姓名','empDescCust.empName')"/>
        </mu-icon-menu>
    ...
    <div id="eqscoll" ref="eqscoll">
      ...
      <mu-table showCheckbox :fixedHeader="true" ref="table" @rowSelection="showItem($event)">
        <mu-thead>
          <mu-tr>
            <mu-th><span style="color: #009688;width: 100%;text-align: center">用户编号</span></mu-th>
            <mu-th><span style="color: #009688">姓名</span></mu-th>
            <mu-th>部门</mu-th>
          </mu-tr>
        </mu-thead>
        <mu-tbody>
          <mu-tr v-for="item in data">
            <mu-td>{{item.empNo}}</mu-td>
            <mu-td style="padding-left: 15px">{{item.empName}}</mu-td>
            <mu-td>{{item.dname}}</mu-td>
          </mu-tr>
        </mu-tbody>
      </mu-table>
      <mu-divider/>
  ...
  export default {
    data () {
      return {
        ...
        searchAll:{
          empNo:localStorage.getItem('empNo'),
          'empDescCust.empName:':"",//员工姓名
          'empDescCust.empNo':"",//员工编号
        },
        dataItem:{},
        seled:0,
        fromurl:""
...
//      进行级联查询
      ...
      getdata:function () {
        ...
        $.ajax({
          url: vm.path+"base/empdesc_query_result.action",
          dataType: "json",
          data: vm.searchmess,
          type: "get",
